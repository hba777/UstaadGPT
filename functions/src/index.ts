...
export const runDailyOrchestration = functions.onSchedule("every 24 hours", async (event) => {
    ...
    for (const userDoc of usersSnapshot.docs) {
      ...
      try {
        ...
        const quizHistorySnapshot = await db.collection("quizAttempts")
          .where("userId", "==", userId).limit(20).get(); // Get last 20 attempts
        ...
        console.log(`Orchestrator recommended action for ${userId}: ${orchestratorResult.agent}`);
        ...
        console.error(`Failed to run orchestrator for user ${userId}:`, error);
