...
export const runDailyOrchestration = functions.onSchedule(
  ...
  async () => {
    ...
    for (const userDoc of usersSnapshot.docs) {
      ...
      console.log(`Processing user: ${userId} (${displayName})`);
      ...
      try {
        ...
        const quizHistory = quizHistorySnapshot.docs
          .map((doc) => doc.data());
        ...
        const agent = orchestratorResult.agent;
        console.log(`Orchestrator recommended action: ${agent}`);
        ...
        console.error(
          `Failed to run orchestrator for user ${userId}:`,
          error
        );
